class WallMerger {
    constructor() {
        this.angleEpsilon = 0.5; //A value in radians that represents the acceptable user error of a wall placed on top of another
        this.distanceEpsilon = 15; // A value that represents the acceptable distance from 2 non touching lines that should still be merged
    }
    static getInstance() {
        if (!WallMerger._instance)
            WallMerger._instance = new WallMerger();
     ...
     * @private
     */
    _calculateCosOfAngleBetween2Walls(firstWall, secondWall) {
     ...
     * @private
     */
    _calculateWallMagnitude(wallEquation) {
     ...
     * @private
     */
    _calculateDotProduct(firstWall, secondWall) {
     ...
     * @private
     */
    _makeNormalVectorForAWall(wallCoords) {
     ...
     * @private
     */
    _checkIntersection(firstWall, secondWall) {
     ...
     * @private
     */
    _isBetween(firstPoint, secondPoint, targetPoint) {
     ...
     * @private
     */
    _distance2(firstPoint, secondPoint) {
     ...
     * @private
     */
    _distToSegmentSquared(targetPoint, firstPointOfLine, secondPointOfLine) {
        const l2 = this._distance2(firstPointOfLine, secondPointOfLine);
        if (l2 === 0)
            return this._distance2(targetPoint, firstPointOfLine);
     ...
     * @private
     */
    _distanceToSegment(targetPoint, firstPointOfLine, secondPointOfLine) {
     ...
     * @private
     */
    _shouldAddToList(targetWall, mainWall, angleBetweenWalls) {
        ...
        if (this._checkIntersection(mainWall, targetWall) && angleBetweenWalls < this.angleEpsilon)
            return true;
            ...
            this._distanceToSegment(wallSecondPoint, mainLineFirstPoint, mainLineSecondPoint) < this.distanceEpsilon)
            return true;
        return false;
    }
    /**
    ...
    _findOverlappingWalls(mainWallCoords, mainWall) {
        ...
        walls.forEach((wall) => {
            ...
            if (this._shouldAddToList(wallCoords, mainWallCoords, angleBetweenWalls))
     ...
     * @private
     */
    _findProjectionOfPointsOnALine(line, point) {
     ...
     * @private
     */
    async _createNewWallFromPointsArray(pointsArray, mainWallData) {
        let wallData;
     ...
     * @private
     */
    createPointObject(pointCoords, pointData) {
        const returnObject = {
            c: pointCoords,
            data: {
                ...pointData
    ...
    _getMainWalls(selectedWalls) {
        ...
        selectedWalls.forEach((wall) => {
            ...
            overlappingWalls.forEach((overlappingWall) => {
                if (this._calculateLineLength(overlappingWall.data.c) < wallLength)
                    shouldMerge.push(overlappingWall);
            });
